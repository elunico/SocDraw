...
describe('utils.js', function () {
  it('should return local ip address', function (done) {
    utils.getLocalIP()
      .then(addr => {
        expect(addr).to.satisfy((a) => {
          return a.indexOf('192') >= 0 || a.indexOf('127') >= 0;
        });
        done();
      })
      .catch(err => {
  ...
  describe('trimAddress', function () {
    it('should not change a address string that is already correct',
      function (done) {
        let s = 'Test string';
    ...
    it('should return an address trimmed', function (done) {
      ...
      expect(utils.trimAddress('::::ffff::0.0.0.0'))
        .to.equal('::::ffff::0.0.0.0');
      done();
    });
  });
});
